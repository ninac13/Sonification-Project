...
bp = Blueprint("visual_trial1", __name__, url_prefix="/visual/trial1")

# Path to your FASTA file inside sonification/data_visual/
SET1_FASTA = os.path.join(
    os.path.dirname(__file__),
    os.pardir, "sonification", "data_visual", "set1.fasta"
)
...
def index():
        ...
        if all([time_taken, markers_used, mutations_found]) and \
           markers_used.isdigit() and mutations_found.isdigit():
            ...
            with open(TRIAL_RESULTS_FILE, "a", newline="") as f:
                ...
                writer.writerow([participant, 1, time_taken, markers_used, mutations_found, misplaced])
    ...
    <body>
      ...
      <div class="container">
        <span class="label">Trial 1 Nonmutated Sequence BELOW</span>
        ...
        <span class="label" style="margin-top: 10px;">Trial 1 Mutated Sequence ABOVE</span>
      </div>
      ...
      <div class="container nav-buttons">
        <button onclick="submitTrial()">Finished Trial 1 and Go to Trial 2 →</button>
      </div>
      <script>
...
<head>
  <meta charset="utf-8">
  <title>Visual Trial 1 Activity</title>
  <style>
    body {
      display: flex;
...
<body>
  <div class="container">
    <h1>Trial 1 Visual DNA Sequences</h1>
  </div><br />                                
  <div class="container">
    ...
    <ul class="instructions">
      ...
      <li>The visual trial 1 will be in the same format as the demonstration activity.</li>
      <li>The same instructions from the demonstration activity apply.</li>
      <li>If you have any questions, let Lea or Nina know BEFORE STARTING THE FIRST TRIAL.</li>
    </ul>
  </div>
    <form method="post" action="{{ url_for('visual_trial1.index') }}?participant={{ participant }}">
      <button type="submit" class="button">
        Start analyzing your first set of DNA Sequences
      </button>
    </form>
</body>
</html>
