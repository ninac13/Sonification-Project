...
def index():
    ...
    if request.method == "POST":
        ...
        correct = 0
        interval_hits = [0] * len(mutation_intervals)


        for m in markers:
           for idx, (start, end) in enumerate(mutation_intervals):
             if start <= m <= end and interval_hits[idx] == 0:
               correct += 1
               interval_hits[idx] = 1
               break
...
<body>
  ...
  <div class="container">
    ...
    <form id="nextForm" method="POST" action="?p={{ participant }}">
      <input type="hidden" name="data">
      <button class="button" type="submit"> Finish Trial 3 →</button>
    </form>
  </div>
